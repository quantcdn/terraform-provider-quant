...
Custom response rules can be imported using the following format:

```bash
terraform import quant_rule_custom_response.resource_name "project_name/rule_uuid"
```

### Example

```bash
terraform import quant_rule_custom_response.test "my-project/12345678-1234-1234-1234-123456789abc"
```
...
- `rule_uuid` is the UUID of the custom response rule in format `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`

### Finding the Rule UUID

To find the rule UUID, you can:
1. Use the QuantCDN dashboard - Look in the rules section for your custom response rule
